...
$p = optional_param('p', 0, PARAM_INT); // Param
...
if($confirm && $dtfid && has_capability('mod/skills:submit', $context)){
	$data = new stdClass;
	$data->id = $dtfid;
	
	if(!$p){
		$data->beinfullreport = 1;
		skills_save('skills_dataform', $data);
		$message = "Prezado(a) ".skills_get_serFullName().", Relatório adicionado à Visão Completa com sucesso!";
		redirect($CFG->wwwroot.'/mod/skills/overview.php?id='.$cm->id.'&perpage=30&page=0', $message, 5);
	}
	
	$data->beinfullreport = 0;
	skills_save('skills_dataform', $data);
	$message = "Prezado(a) ".skills_get_serFullName().", Relatório removido da Visão Completa com sucesso!";
	redirect($CFG->wwwroot.'/mod/skills/overview.php?id='.$cm->id.'&perpage=30&page=0', $message, 5);
