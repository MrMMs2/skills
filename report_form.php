 ...
 */
class mod_skills_report_form extends moodleform {
	...
	public function definition() {
		...
		$mform->addElement('html', "<div class='qtdeservers_intructions hide_instructions'>
										<div class='alert alert-info' role='alert'>
											<p><i class='fa fa-info-circle'></i> <strong>Quantidade de Servidores: </strong> Incluir aqueles com vínculo: servidores efetivos (independentemente de ocupação de cargo ou função comissionados), requisitados, cedidos e comissionados.</p>
										</div>
									</div>");
		...
		// Campo methodology (Instrumentos utilizados)
		...
		$mform->addElement('html', "<div id='id_methodology'>
				...
				<div class='items_questions'>
					<input type='checkbox' id='ck1' name='methodology[]' value='AVALIAÇÃO DE DESEMPENHO POR COMPETÊNCIAS'> <label for='ck1'>AVALIAÇÃO DE DESEMPENHO POR COMPETÊNCIAS</label><br />
					<input type='checkbox' id='ck2' name='methodology[]' value='AVALIAÇÃO DE DESEMPENHO SEM COMPETÊNCIAS'> <label for='ck2'>AVALIAÇÃO DE DESEMPENHO</label><br />
					<input type='checkbox' id='ck3' name='methodology[]' value='LEVANTAMENTO DE NECESSIDADE DE TREINAMENTO'> <label for='ck3'>LEVANTAMENTO DE NECESSIDADE DE TREINAMENTO</label><br />
					<input type='checkbox' id='ck4' name='methodology[]' value='HISTÓRICO DOS ANOS ANTERIORES'> <label for='ck4'>HISTÓRICO DOS ANOS ANTERIORES</label><br />
					<input type='checkbox' id='ck5' name='methodology[]' value='PLANEJAMENTO ESTRATÉGICO'> <label for='ck5'>PLANEJAMENTO ESTRATÉGICO</label><br />
					<input type='checkbox' id='ck6' name='methodology[]' value='ANÁLISE DOS MACROPROCESSOS'> <label for='ck6'>ANÁLISE DOS MACROPROCESSOS</label><br />
					<label for='ck7'>OUTRO(S): </label><br />
					<textarea id='ck7' onkeyup='this.value = this.value.toUpperCase()' class='textarea' name='methodology[]'></textarea>
				</div> 
			</div>");
		...
		$mform->addElement('html', "<div class='items_questions'>
					<input type='checkbox' id='courses360h' name='formativeactionsyearprevious[]' value='CURSOS COM MENOS DE 360h'> <label for='courses360h'>CURSOS COM MENOS DE 360h</label><br />
				</div>");
		...
		foreach ($areas as $area){
			...
			$areas_html .= "<div class='panel panel-default'>";
				...
				$areas_html .= "<div id='collapse".$cont."' class='panel-collapse collapse ".$classin."' role='tabpanel' aria-labelledby='heading".$cont."'>";
					$areas_html .= "<div class='panel-body'><p>{$area->description}</p> ".skills_generate_table_areas($area->id)."</div>"; // description
				$areas_html .= "</div>";
			$areas_html .= "</div>";
		...
		foreach ($trainings as $training){
			$training_html .= "<div class='items_questions'>
						<input onclick='return training_verify({$training->id});' type='checkbox' id='tgn_{$training->id}' name='formativeactionsyearprevious[]' value='{$training->name}'> <label for='tgn_{$training->id}'>{$training->name}</label><br />
					</div>";
			if(strtoupper(trim($training->name)) == strtoupper(trim('OUTRO(S): '))){
				$training_html .= skills_generate_textarea('formativeactionsyearprevious[]','training ', 'training'.$training->id);
			}else{
				$training_html .= skills_generate_table_training($training);
			}
		}
		...
		$mform->addElement('html', "<div id='id_trainingactionsociety'>
					<div class='question child'> PREENCHA A TABELA ABAIXO: </div>".skills_get_table_trainingactionsociety().
				"</div>");
		...
		$mform->addElement('html', "<div id='id_structuretraining'>
				...
				<div class='items_questions'>
					<input type='checkbox' id='ckstruct1' name='structuretraining[trainingrom]' value='SALAS DE TREINAMENTO'> <label for='ckstruct1'>SALAS DE TREINAMENTO</label><br />
					<input type='checkbox' id='ckstruct2' name='structuretraining[computerlab]' value='LABORATORIO DE INFORMATICA'> <label for='ckstruct2'>LABORATÓRIO DE INFORMÁTICA</label><br />
					<input type='checkbox' id='ckstruct3' name='structuretraining[library]' value='BIBLIOTECA'> <label for='ckstruct3'>BIBLIOTECA</label><br />
					<input type='checkbox' id='ckstruct4' name='structuretraining[auditorium]' value='AUDITORIO'> <label for='ckstruct4'>AUDITÓRIO</label><br />
					<input type='checkbox' id='ckstruct5' name='structuretraining[studios]' value='ESTUDIOS DE GRAVACAO'> <label for='ckstruct5'>ESTÚDIOS DE GRAVAÇÃO</label><br />
					<input type='checkbox' id='ckstruct6' name='structuretraining[learningplatform]' value='PLATAFORMA DE APRENDIZAGEM'> <label for='ckstruct6'>PLATAFORMA DE APRENDIZAGEM</label><br />
					".skills_get_table_structuretraining()."
				</div>
			</div>");
		...
		$mform->addElement('html', "<div id='id_runvalbyformaction'>
				...
				<div class='items_questions'>
					".skills_get_table_runvalbyformaction()."
				</div>
			</div>");
		...
		$mform->addElement('html', "<div id='id_typeevaluation'>
				...
				<div class='items_questions'>
					<input type='checkbox' id='tpev1' name='typeevaluation[]' value='REAÇÃO'> <label for='tpev1'>REAÇÃO <span class='label label-info'>(avalia a impressão dos servidores sobre o conteúdo, instrutores, recursos, ambiente, etc).</span></label><br />
					<input type='checkbox' id='tpev2' name='typeevaluation[]' value='APRENDIZAGEM'> <label for='tpev2'>APRENDIZAGEM <span class='label label-info'>(examina se servidores absorveram os conhecimentos e aperfeiçoaram habilidades e atitudes).</span></label><br />
					<input type='checkbox' id='tpev3' name='typeevaluation[]' value='APLICAÇÃO'> <label for='tpev3'>APLICAÇÃO <span class='label label-info'>(identifica se estão utilizando em seu trabalho os conhecimentos, habilidades e atitudes apreendidos).</span></label><br />
					<input type='checkbox' id='tpev4' name='typeevaluation[]' value='RESULTADO'> <label for='tpev4'>RESULTADO <span class='label label-info'>(analisa se a ação formativa contribuiu para o alcance da estratégia do órgão).</span></label><br />
					<label for='tpev5'>OUTROS: </label> <br /> 
					<textarea id='tpev5' onkeyup='this.value = this.value.toUpperCase()' class='textarea' name='typeevaluation[]'></textarea>
				</div>
			</div>");
		...
		$mform->addElement('html', "<div id='id_formativeactionsyearcurrent'>
				...
				<div class='items_questions'>
					<input type='checkbox' id='fa1' name='formativeactionsyearcurrent[]' value='CURSOS PRESENCIAIS COM MENOS DE 360H'> <label for='fa1'>CURSOS PRESENCIAIS COM MENOS DE 360H</label><br />
					<input type='checkbox' id='fa2' name='formativeactionsyearcurrent[]' value='CURSOS EAD COM MENOS DE 360H'> <label for='fa2'>CURSOS EAD COM MENOS DE 360H</label><br />
					<input type='checkbox' id='fa3' name='formativeactionsyearcurrent[]' value='PÓS-GRADUAÇÃO'> <label for='fa3'>PÓS-GRADUAÇÃO</label><br />
					<input type='checkbox' id='fa4' name='formativeactionsyearcurrent[]' value='PALESTRAS'> <label for='fa4'>PALESTRAS</label><br />
					<input type='checkbox' id='fa5' name='formativeactionsyearcurrent[]' value='CONGRESSOS'> <label for='fa5'>CONGRESSOS</label><br />
					<input type='checkbox' id='fa6' name='formativeactionsyearcurrent[]' value='ENCONTROS'> <label for='fa6'>ENCONTROS</label><br />
					<input type='checkbox' id='fa7' name='formativeactionsyearcurrent[]' value='SEMINÁRIOS'> <label for='fa7'>SEMINÁRIOS</label><br />
					<input type='checkbox' id='fa8' name='formativeactionsyearcurrent[]' value='FÓRUNS'> <label for='fa8'>FÓRUNS</label><br />
					<input type='checkbox' id='fa9' name='formativeactionsyearcurrent[]' value='WORKSHOPS'> <label for='fa9'>WORKSHOPS</label><br />
					<input type='checkbox' id='fa10' name='formativeactionsyearcurrent[]' value='GRADUAÇÕES'> <label for='fa10'>GRADUAÇÕES</label><br />
					<label for='tpev9'>OUTRAS: </label><br />
					<textarea id='tpev9' onkeyup='this.value = this.value.toUpperCase()' class='textarea' name='formativeactionsyearcurrent[]' /></textarea>
				</div>
			</div>");
		...
		$mform->addElement('textarea', 'comments', get_string('comments', 'skills'), array('style'=>'width: 104.5%;', 'rows'=>5, 'onkeyup'=>'this.value = this.value.toUpperCase()'));
